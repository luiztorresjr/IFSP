...
ALTER TABLE local ADD FOREIGN KEY (cod_cep) REFERENCES cep(cod_cep);
ALTER TABLE excursao ADD FOREIGN KEY (cod_local) REFERENCES local(cod_local);
...
FROM	aluno AS a,
	...
	local AS l,
	excursao AS e
WHERE a.cod_aluno = ae.cod_aluno AND ae.cod_excursao=e.cod_excursao AND l.cod_local=e.cod_local;
...
SELECT	nome,
	descricao
FROM	excursao AS e
LEFT JOIN aluno_excursao AS ae
	ON(ae.cod_excursao=e.cod_excursao)
LEFT JOIN aluno AS a
	ON(a.cod_aluno = ae.cod_aluno)
LEFT JOIN local AS l
	ON(l.cod_local=e.cod_local)
...
UPDATE excursao
SET preco = preco*1.2
WHERE cod_local IN(SELECT e.cod_local 
		   FROM excursao e, aluno_excursao ae, cep c,local l
		   WHERE e.cod_excursao=ae.cod_excursao AND e.cod_local = l.cod_local AND l.cod_cep=c.cod_cep AND pais <>'Brasil'
		  );
...
FROM	excursao e,
	cep c,
	local l
WHERE  e.cod_local=l.cod_local AND c.cod_cep=l.cod_cep